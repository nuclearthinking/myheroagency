...
import com.nuclearthinking.myheroagency.controller.loader.file.CustomI18NBundleLoader;
import com.nuclearthinking.myheroagency.controller.loader.file.JsonLoader;
...
@Slf4j(topic = "AssetsManager")
public final class AssetsManager implements Disposable, AssetErrorListener {
    ...
    public void loadGroup(@NonNull final String groupName) {
        ...
        if (assets != null) {
            for (val asset : assets) {
                ...
                log.debug("AssetsManager {} added to loading queue", asset.path);
            }
        } else {
        ...
        if (assets != null) {
            for (val asset : assets) {
                if (manager.isLoaded(asset.path, asset.type)) {
                    ...
                    log.debug("AssetsManager {} added to unload queue", asset.path);
                }
            }
        } else {
    ...
    private class Assets {
        ...
        public Assets(@NonNull final String type, @NonNull final String path) {
            try {
                ...
                log.error("AssetsManager type {} not found", type);
            }
        }
    }
